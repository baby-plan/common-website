define(["QDP"], function (QDP) {
  ...
  return {
    ...
    /** 卸载模块 */
    destroy: function () {
      $(document).off('click', '.add-new');
      $(document).off('click', '.content-toolbar>.btn-delete');
      $(document).off('click', '.content-toolbar>.btn-selectall');
      $(document).off('click', '.content-toolbar>.btn-selectopp');
      $(document).off('click', 'form ul>li:not(:first)>a');
      $(document).off('click', '#list>div');
      console.log('卸载商户信息管理注册的事件处理程序');
    },
