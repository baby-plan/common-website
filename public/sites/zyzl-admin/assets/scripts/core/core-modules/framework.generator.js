...
define(['jquery', 'cfgs', 'API',
  ...
  'moment', 'bootstrapValidator', 'jquery.tmpl', 'jquery.tmplPlus'],
  function ($, cfgs, api, ajax, base64, dialog, layout, util, form, table, row, cell) {
    "use strict";
      ...
      if (hasFilter) {// 处理查询条件区块内容及事件
        ...
        $.each(plugin_option.columns, function (index, column) {
          ...
          if (column.custom) {
            ...
            form.initDateRange(column.name + "_1", column.name + "_2", column.name + "_range");
    ...
    var save_click = function (e) {
      ...
      ajax.get(requesturl, options, function (json) {
        layout.back();
      });
    }
    ...
    var reomve_click = function () {
      ...
      var confirmCallback = function (result) {
        ...
        ajax.get(plugin_option.apis.delete, options, function (json) { table.reload(); });
      }
    ...
    var _search = function () {
      ...
      var parsefn = function (tr, index, totalindex, item) {
        ...
        $.each(columns, function (column_index, column) {
          ...
          if (column.name == "_index") { // 序号
            ...
            if (column.customAction && typeof column.customAction == 'function') {
      ...
      table.load({ "api": plugin_option.apis.list, "parsefn": parsefn, "args": options });
    }
