define(['jquery', 'cfgs', 'ajax'], function ($, cfgs, ajax) {
  ...
  return {
    'name': 'plugins.layout.table',
    'version': '0.0.0.1',
    //公共数据处理:回调parsefn填充数据并且生成分页按钮
    "load": common_load_data,
    "reload": re_common_load_data,
    //根据缓存数据索引获取数据内容
    "getdata": getData,
    "render": rander,
    "export": function (type, target) {
      console.error("导出" + type + ":尚未实现" + target);
    },
    "exportCSV": function (target) {
      console.error("导出CSV:尚未实现" + target);
    }
  };
