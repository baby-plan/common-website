...
define(
  [
    ...
    'generator-plugins/select-map/index',
    'generator-plugins/upload-image/index',
  ...
  function (
      ...
      "init": function (options) {
        ...
        $.each(plugin_option.columns, function (index, column) {
          ...
          if (column.dict && column.dict != '') {
            ...
            column.plugin = plugin_selectmap;
          } else if (column.type == 'upload') {
            column.plugin = plugin_uploadimage
            // } else if (column.type) {
    ...
    var initEditView = function (data) {
      ...
      $.each(plugin_option.columns, function (index, column) {
        ...
        if (column.type == "map") {
          // 处理地图类型字段
          requirePlugins('select-map', column, data, column.events);
        } else if (column.type == "image") {
          // 处理图片类型字段
          requirePlugins('upload-image', column, data, column.events);
        } else if (column.custom) {
          var columndiv = $("#" + column.name).parent();
          columndiv.empty();
          column.custom(column, columndiv, value);
        } else {
          column.plugin.editor(column, value);
        }
