  ...
  module.init = (el) => {
    ...
    option = {
      ...
      tooltip: {
        ...
        formatter: function (params) {
          ...
          var types = params[0].trim().split(' ');
          var text = data.name + "<br />";
          if (types.length == 2) {

            text += types[0] + "总数：" + data.valueMap[0];
            text += "<br />" + types[1] + "总数：" + data.valueMap[1];
          } else {
            text += types[0] + "总数：" + data.value;
          }
          return text;
        }
        // formatter: '点击进入该省<br/>{b}{value}'
      },
      toolbox: {
        show: true,
        orient: 'vertical',
    ...
    chart.on('click', function (param) {
      ...
      if (mapType.indexOf(param.name) > 0) {
        mt = param.name
        option.title.text = mt + "订单概述";
        option.title.subtext = '点击任意区块返回全国订单概述';
      } else {
        mt = 'china';
        option.title.text = "全国订单概述";
        option.title.subtext = '点击区块进入省级订单概述';
      }
