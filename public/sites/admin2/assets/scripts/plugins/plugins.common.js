...
(function (app) {
    ...
    if (hasFilter) {// 处理查询条件区块内容及事件
      ...
      $.each(plugin_option.columns, function (index, column) {
        ...
        if (column.custom) {
          ...
          App.form.initDateRange(column.name + "_1", column.name + "_2", column.name + "_range");
  ...
  var save_click = function (e) {
    ...
    $.each(plugin_option.columns, function (index, column) {
      ...
      if (column.type == "date" || column.type == "datetime") {
        options[column.name] = App.util.stringToUTC(value);
      } else if (column.multiple) {
        options[column.name] = value.join();
      } else {
        if (column.base64) {
          options[column.name] = App.base64.encode(value);
        } else {
    ...
    $.each(plugin_option.columns, function (index, column) {
      ...
      if (column.custom) {
        ...
        if (column.type == "date") {
          ...
          if (value) {
            $("#" + column.name).val(App.util.utcTostring(value, cfgs.options.defaults.dateformat2));
          }
        } else {
          ...
          if (value) {
            $("#" + column.name).val(App.util.utcTostring(value, cfgs.options.defaults.datetimeformat));
          }
        }
        ...
        App.form.initDict(column.dict, $("#" + column.name), value, column.multiple);
  ...
  var reomve_click = function () {
    ...
    var confirmCallback = function (result) {
      ...
      App.ajax(plugin_option.apis.delete, options, function (json) { App.table.reload(); });
    }
    App.confirm("确定删除记录?", confirmCallback);
  }
  /** 排序函数：比较filterindex */
    ...
    $.each(plugin_option.columns, function (index, column) {
      ...
      if (column.filter == "daterange") {
        ...
        if (starttime && starttime != "") {
          options[column.name + "_1"] = App.util.stringToUTC(starttime);
        }
        var endtime = $("#" + column.name + "_2").val();
        if (endtime && endtime != "") {
          options[column.name + "_2"] = App.util.stringToUTC(endtime) + 86400;
        }
      } else if (column.multiple) {
    ...
    var parsefn = function (tr, index, totalindex, item) {
      ...
      $.each(columns, function (column_index, column) {
        ...
        if (column.name == "_index") { // 序号
          ...
          td = App.row.addUTCText(tr, value, cfgs.options.defaults.dateformat2);
        } else if (column.type == "datetime") { // 显示时间类型
          td = App.row.addUTCText(tr, value, cfgs.options.defaults.datetimeformat);
        } else if (column.dict && column.dict != "") { // 数据字典类型 
          td = App.row.addDictText(tr, column.dict, value, column.multiple);
        } else if (column.primary) { // 主键，不显示
          if (column.display) {
            td = App.row.addText(tr, value);
          }
        } else {
          if (column.base64) {
            value = App.base64.decode(value);
          ...
          if (column.overflow != undefined && column.overflow > 0 && value.length > column.overflow) {
            td = App.row.addText(tr, value.substring(0, column.overflow) + "..."); // 文本类型
            td.attr("title", value);
          } else {
            App.row.addText(tr, value); // 文本类型
          }
        }
      });
    }

    App.table.load({ "api": plugin_option.apis.list, "parsefn": parsefn, "args": options });
  }
  /** 重置页面输入项 */
  var _reset = function () {
    $.each(plugin_option.columns, function (index, column) {
