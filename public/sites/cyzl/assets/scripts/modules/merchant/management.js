define(["QDP"
  ...
  var approval_click = function () {
    var data = QDP.table.getSelect();
    if (data == null) {
      QDP.alertText('请选择需要通过审核的商户（仅选择1条记录）');
      return;
    }
  ...
  var reject_click = function () {
    var data = QDP.table.getSelect();
    if (data == null) {
      QDP.alertText('请选择需要驳回审核的商户（仅选择1条记录）');
      return;
    }
    ...
    if (selector.length == 1) {
      ...
      if (data) {
        ...
        if (data.publish_status == 1) {
          $(".btn-unpublish").removeAttr('disabled');
        }
      }
      $(".btn-chgpwd").removeAttr('disabled');

    } else if (selector.length == 0) {
      $(".btn-reject").attr('disabled', "true");
      $(".btn-approval").attr('disabled', "true");
      $(".btn-chgpwd").attr('disabled', "true");
      $(".btn-publish").attr('disabled', "true");
      $(".btn-unpublish").attr('disabled', "true");
    } else {
      $(".btn-reject").attr('disabled', "true");
      $(".btn-approval").attr('disabled', "true");
      $(".btn-chgpwd").attr('disabled', "true");
  ...
  var initAction = () => {
    ...
    if (powers.indexOf('approval') > -1) {
      QDP.form.appendAction(actionContainer, "btn-approval", approval_click);
    }    // TODO: 权限校验-MCH_MANAGEMENT_REJECT
    if (powers.indexOf('reject') > -1) {
      QDP.form.appendAction(actionContainer, "btn-reject", reject_click);
    }
    // TODO: 权限校验-MCH_MANAGEMENT_PUBLISH
    if (powers.indexOf('publish') > -1) {
      QDP.form.appendAction(actionContainer, "btn-publish", publish_click);
    }
    // TODO: 权限校验-MCH_MANAGEMENT_UNPUBLISH
    if (powers.indexOf('unpublish') > -1) {
      QDP.form.appendAction(actionContainer, "btn-unpublish", unpublish_click);
    }
    // TODO: 权限校验-MCH_MANAGEMENT_RESETPASSWORD
    if (powers.indexOf('resetpassword') > -1) {
      QDP.form.appendAction(actionContainer, "btn-chgpwd", resetpassword_click);
    }
    ...
    "init": function () {
      var options = {
        ...
        "columns": [
          ...
          {
            name: "map-location", text: "地图位置", type: 'map', edit: true,
            lngcolumn: 'location_lon', latcolumn: 'location_lat', locationcolumn: 'mch_addr'
          },
          {
