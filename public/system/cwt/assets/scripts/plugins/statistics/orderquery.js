//统计中心插件-指令查询模块：
//app.plugins.statistics.orderquery
//scripts/plugins/statistics/orderquery.js
(function (app) {
  // 初始化时间选择控件
  ...
  var getOptions = function (parentView) {
    ...
    var ids = [];
    $.each($("#treeview", parentView).jstree().get_checked(true), function (index, item) {
      if (item.children.length == 0) {
        ids.push(item.original.sid);
      }
    });
    ...
    options.sIds = App.util.urlencode(ids.join(','));
    return options;
  };
  ...
  App.plugins.statistics.orderquery = {
    init: function (parent) {
      ...
      $("#doSearch", parentView).on("click", function () {
        var url = bus_cfgs.reportStatistics.commandStatapi;
