define(["QDP"
  ...
  var submit_click = function () {
    ...
    if (data == null) {
      QDP.alert('请选择需要提交审核的公告（仅选择1条记录）');
      return;
    }

    QDP.confirm("确定提交审核该公告?", function (result) {
      if (!result) {
        return;
      }
      console.debug("MCH_NOTICE_SUBMIT");
    });
  }
  ...
  var publish_click = function () {
    ...
    QDP.confirm("确定发布该商户?", function (result) {
      ...
      console.debug("MCH_NOTICE_PUBLISH");
  ...
  var unpublish_click = function () {
    var data = QDP.table.getSelect();
    QDP.confirm("确定取消该公告的发布状态?", function (result) {
      if (!result) {
        return;
      }
  ...
  var audit_click = function () {
    ...
    QDP.form.openWidow({
      ...
      'width': '800px',
      'height': '600px',
      'url': 'views/business/notice-audit.html',
    ...
    "init": function () {
      var options = {
        ...
        "columns": [
          ...
          { "name": "type", "text": "公告类型", "dict": "noticetype", "edit": true, "filter": true },
