...
(function (app) {
  ...
  app.util = {
    "parseURL": (url) => { return _parseURL(url); },
    "stringToUTC": (string) => { return stringToUTC(string); },
    "dateformat": (date, format) => { return datetimeToString(date, format); },
    "utcTostring": (timestamp, format) => { return utcTostring(timestamp, format); },
    "utcToCN": (timestamp) => { return utcToCN(timestamp); },
    "jsonToString": (json) => { return JSON.stringify(json) }
  };
})(App);